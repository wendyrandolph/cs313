...
function generateView($results, $images)
{
    ...
    foreach ($results as $item) {
        ...
        $view .= '<div>';
        $view .= "<form action='/web/week_3/index.php?action=addToCart&pId=$item[pId]&title=$item[title]&desc=$item[desc]&price=$item[price]&quan=$item[quan]&image=$item[image]' method=POST>";
        ...
        $view .= '</div>';
        $view .= '<div id="addto">';
        $view .= "<input 'type='button' id=addme value='Add To Cart'>";
        $view .= '</div>';
